...
function targetStatusClass(s){
  if(s==='click_confirmed'||s==='case_verified'||s==='visual_confirmed')return 'bg-g';
...
function targetLoad(){
  ...
  fetch('/api/target/list?limit=200&keyword='+encodeURIComponent(kw)).then(function(r){return r.json();}).then(function(d){
    targetState.items=d.tasks||[];
    var list=document.getElementById('targetList');
    document.getElementById('targetStats').textContent=(d.returned||0)+'/'+(d.total||0)+' 个目标';
    if(!targetState.items.length){
      list.innerHTML='<div style="padding:12px;text-align:center;color:#999;">暂无目标，点击“添加”录入一个目标。</div>';
      targetRender(null);
      return;
    }
...
function targetQuickAdd(){
  ...
  fetch('/api/target/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({targetName:name,pageHint:page||'',role:role||'',priority:'P1'})})
    .then(function(r){return r.json();}).then(function(d){if(d.success){targetState.selected=d.task;targetLoad();ml('目标已添加');}else{ml('添加目标失败: '+(d.error||''),'e');}});
}
function targetGenerateRuntime(){
  var limit=prompt('最多生成多少个当前页目标？', '80');
...
function targetRender(t){
  ...
  h+='<div style="margin-top:5px;display:flex;gap:3px;flex-wrap:wrap;"><button class="btn btn-sm" style="background:#00695C;color:#fff;" onclick="targetRunNext()">下一步：'+targetNextText(t)+'</button><button class="btn btn-sm" onclick="targetMatchCandidates()">匹配候选</button><button class="btn btn-sm" onclick="targetIgnore()">忽略</button></div>';
...
function targetRefreshSelected(){
  var id=targetState.selected&&targetState.selected.targetId;
  fetch('/api/target/list?limit=200').then(function(r){return r.json();}).then(function(d){targetState.items=d.tasks||[];if(id){targetSelect(id);}targetLoad();});
}
...
function targetSelectCandidate(pathEncoded){
  ...
  fetch('/api/target/select_candidate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({targetId:t.targetId,draftPath:p})})
    .then(function(r){return r.json();}).then(function(d){if(d.success){targetState.selected=d.task;targetRender(d.task);ml('候选已选择');}else{ml('选择失败: '+(d.error||''),'e');}});
}
...
function targetCall(url,label){
  ...
  fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({targetId:t.targetId})})
    .then(function(r){return r.json();}).then(function(d){targetSetResult(d.success?(label+'完成'):('失败: '+(d.error||d.status||'')),!!d.success);targetRefreshSelected();});
}
function targetIgnore(){
  ...
  fetch('/api/target/ignore',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({targetId:t.targetId,reason:'user_ignored'})})
    .then(function(r){return r.json();}).then(function(d){if(d.success){targetRefreshSelected();ml('目标已忽略');}else{ml('忽略失败','e');}});
}
function ldDrafts(){
  var kw = document.getElementById('rkw').value;
