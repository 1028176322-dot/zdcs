...
class RuntimeCodeMapper:
    ...
    def identify_current_ui(self):
        ...
        def extract_node_names(node, depth=0):
            """递归提取节点名称"""
            if not node:
                return
            ...
            # 只关注可能的UI类节点 (通常是最顶层的节点)
            if depth <= 3:  # 只取前3层
                runtime_node_names.add(node_name.lower())
            
            # 递归处理子节点
            children = node.get('children', [])
            for child in children:
                extract_node_names(child, depth + 1)
        
        extract_node_names(self.runtime_ui_tree)
        
        print(f"提取到 {len(runtime_node_names)} 个UI节点名称")
        ...
        for ui_class in self.static_data:
            ...
            # 获取该类的所有可点击元素的likely_name_in_ui
            ui_names = set()
            ...
            # 计算匹配度 (交集大小)
            match_count = len(ui_names & runtime_node_names)
            
            #  also check if class name matches any runtime node name
    ...
    def _generate_html_report(self, output_path, timestamp):
        ...
        html_content += """
            <hr>
            <p><em>报告生成工具: Runtime Code Mapper v1.0</em></p>
        </body>
        </html>
        """
